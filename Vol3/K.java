...
public class K {
    ...
    static public int[] res(Graph g, int s, int dmin, int dmax){
        int[] dist = new int[g.num_vertices()+1];
        for(int i=0;i<g.num_vertices()+1;i++) dist[i] = Integer.MAX_VALUE;
        dist[s] = 0;
        ...
        while (!q.isEmpty()) {
            ...
            for(Edge ed : g.adjs_no(v)){
                if(dist[ed.endnode()] > dist[v]+ed.value() ){
                    dist[ed.endnode()] = dist[v]+ed.value();
    ...
    static public void main(String[] args){
        ...
        int c = in.nextInt(), dmin = in.nextInt(), dmax = in.nextInt();
        int n_nos = in.nextInt(), ramos = in.nextInt();
        Graph g = new Graph(n_nos);

        for(int i=0;i<ramos;i++){
            int u = in.nextInt(), v = in.nextInt(), d = in.nextInt();
            g.insert_new_edge(u, v, d);
            g.insert_new_edge(v, u, d);
        }
        int[] dist = res(g,c,dmin,dmax);

        int count = 0;
        String txt = "";
        for(int i=1;i<n_nos+1;i++){

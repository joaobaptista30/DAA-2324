...
public class J {
    ...
    static public void res(Graph g, int s){
        ...
        while ( !q.isEmpty()) {
            int v = q.extractMin();
            for(Edge ed : g.adjs_no(v)){
                if((ed.value() + dist[v]) < dist[ed.endnode()]){
                    dist[ed.endnode()] = ed.value() + dist[v];
                    q.decreaseKey(ed.endnode(),dist[ed.endnode()]);
                }
            }
        }
        ...
        for(int i=1;i<g.num_vertices();i++){
            ...
            for(int j=1;j<g.num_vertices()+1;j++){
                if(dist[j]<min){
                    id=j; min = dist[j];
                }
            }
    ...
    static public void main(String[] args){
        ...
        int n_lojas = in.nextInt(), id_destino = in.nextInt();
        int a = in.nextInt();
        Graph g = new Graph(n_lojas);

        while(a != -1){
            int b = in.nextInt(), dist = in.nextInt();
            g.insert_new_edge(a, b, dist);
            g.insert_new_edge(b, a, dist);
            a = in.nextInt();
        }
        res(g,id_destino);
